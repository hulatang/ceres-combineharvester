...
## 1.2.5 Ceres blockchain 2021-08-27

### Fixed

- Fixed errors in the GUI install script.

## 1.2.4 Ceres blockchain 2021-08-26
...
- Added a py.typed file that allows other projects that pip install ceres-blockchain to type check using our functions with mypy.
- Added an RPC for coin records by multiple coin names.
- Enabled querying AAAA records for DNS Introducer.
- We now set the version for the GUI when doing a manual install using the install-gui.sh script. Uses a python helper to get the version of the ceres install and then converts it into proper npm format and puts that into package.json.
...
- Thanks @altendky for changing the default to paginate to ceres wallet get_transactions to address cases such as piping and output redirection to a file where the command previously just hung while waiting for the user to press c for the next page.
...
- Switched IP address lookup to first use Ceres's service ip.ceres.net.
...
## 1.2.3 Ceres blockchain 2021-07-26
...
- Thanks @mishan for your change to 'ceres plotnft show' to display Percent Successful Points.
...
## 1.2.2 Ceres blockchain 2021-07-13

### Fixed

- Converted test_rom.py to use pytest and fixed test_singleton.
- Thanks to @yshklarov for help fixing [#7273](https://github.com/Ceres-Network/ceres-blockchain/issues/7273), which bundled CA store to support pools for some farming systems, including M1 Apple computers. This enables those machines to properly connect to pools, and fixes the issue.

## 1.2.1 Ceres blockchain 2021-07-12
...
## 1.2.0 Ceres blockchain 2021-07-07

### Added

- Portable pooled plots are now available using our new plot NFT. These allow you to plot new plots to an NFT that can either self farm or join and leave pools. During development there were changes to the plot NFT so portable pool plots (those made with `-c` option to `ceres plots create`) using code from before June 25th are invalid on mainnet.
OG plots made before this release can continue to be farmed side by side with the new portable pool plots but can not join pools using the official pooling protocol. You can learn more as a farmer by checking out the [pool user guide](https://github.com/Ceres-Network/ceres-blockchain/wiki/Pooling-User-Guide). Pool operators and those wanting to understand how the official pooling protocol operates should check out our [pooling implementation reference repository](https://github.com/Ceres-Network/pool-reference). If you plan to use plot NFT, all your farmers and harvesters must be on 1.2.0 to function properly for portable pool plots.
...
- `ceres farm summary` and the GUI now use a new RPC endpoint to properly show plots for local and remote harvesters. This should address issues #6563, #5881, #3875, #1461.
- `ceres configure` now supports command line updates to peer count and target peer count.
...
- All Ceres repositories now automatically build M1 wheels and create a new MacOS M1 native installer.
- New CLI command `ceres plotnft` to manage pools.
...
- Correctly display private key in `ceres keys show`.
- Thanks to @gldecurtins for removing a default printout of the private key mnemonic in `ceres keys show`.
...
- `ceres farm summary` better handles wallet errors.
...
### 1.1.7 Ceres Blockchain 2021-06-05
...
## 1.1.6 Ceres Blockchain 2021-05-20
...
- We added a simple profiler to track performance times in the application (see /ceres/util/profiler.py for instructions).
...
- Added configuration options for moving an install to testnet and back (use `ceres configure -t true|false`).
...
## 1.1.5 Ceres Blockchain 2021-05-09
...
## 1.1.4 Ceres Blockchain 2021-05-04

### Changed

- Secret wallet key is hidden by default in `ceres keys show`. Use `ceres keys show --show-mnemonic-seed` for private keys.
...
## 1.1.3 Ceres Blockchain 2021-05-01
...
## 1.1.2 Ceres Blockchain 2021-04-24
...
## 1.1.1 Ceres Blockchain 2021-04-21
...
## 1.1.0 Ceres Blockchain 2021-04-21

### Added

- This fork release includes full transaction support for the Ceres Blockchain. Transactions are still disabled until 5/3/2021 at 10:00AM PDT. It is hard to overstate how much work and clean up went into this release.
...
## 1.0.5 Ceres Blockchain 2021-04-14
...
## 1.0.4 Ceres Blockchain 2021-04-12
...
- The `src` directory in ceres-blockchain has been changed to `ceres` to avoid namespace collisions.
- GUI install builds have been simplified to rely on one `.spec` file in `ceres/`
...
## 1.0.3 Ceres Blockchain 2021-03-30

### Added

- This is a minor bug fix release for version 1.0.2
- You should review the [release notes for v1.0.2](https://github.com/Ceres-Network/ceres-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.

### Fixed

- An incorrect merge brought in unreleased features and broke `ceres keys`.
- Omitted from the 1.0.2 changelog, we fixed one crash in harvester with the release of chiapos 1.0.0 as well.

## 1.0.2 Ceres Blockchain 2021-03-30

### Added

- We have released version 1.0.0 of [chiapos](https://github.com/Ceres-Network/chiapos). This includes a 20% speed increase for bitfield plotting compared to the previous version on the same machine. In many cases this will mean that bitfield plotting is as fast or faster than non bitfield plotting.
...
- We have added Dutch and Polish to supported translations. Thanks @psydafke, @WesleyVH, @pieterhauwaerts, @bartlomiej.tokarzewski, @abstruso, @feel.the.code, and @Axadiw for contributions to [translations on Crowdin](https://crowdin.com/project/ceres-blockchain).
...
- @jespino changed `ceres keys show` to require the `--show-mnemonic-seed` before it displays your 24 work private key mnemonic.
- We decreased the size of the block cache in node to perform better with longer chains.
- You can now add a private key mnemonic from a file with `ceres keys show`.
...
## 1.0.1 Ceres Blockchain 2021-03-23

### Added

- There is now a simple progress bar on the GUI Plot page and when you view the log from the three dots on the right.
- Users must now explicitly set the `--show-mnemonic-seed` flag to see their private keys when running `ceres keys show`.
- We are now building Linux GUI installers. These should be considered beta quality for now.
- Translations now available for German, Traditional Chinese, and Danish. Thanks to @Dravenex, @MaestroOnICe, @loudsyncro, @loppefaaret, @thirteenthd, @wong8888, @N418, and @swjz for all the translation help. You to can translate at our [Crowdin project](https://crowdin.com/project/ceres-blockchain/).
...
- Thank you to @L3Sota for adding a Japanese translation via our [Crowdin project](https://crowdin.com/project/ceres-blockchain).
...
- This is a hard fork/breaking change from RC6/7. TXCH Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.ceres.net/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `ceres farm summary` will now display TXCH or XCH as appropriate.
...
- `ceres farm summary` displayed some incorrect amounts.
...
- `ceres netspace` now defaults to 1000 blocks to mirror the GUI.
...
- There was a typo in `ceres netspace`. Thanks @altendky.
- There was a typo in `ceres plots`. Thanks @adamfiddler.

### Known Issues

- Some users can't plot in the GUI in MacOS Big Sur - especially on M1. See issue [1189](https://github.com/Ceres-Network/ceres-blockchain/issues/1189)
...
- Remove `ceres plots "-s" "--stripe_size"` and the strip size setting in the Advanced section of the GUI. We now always use the best default of 64K for the GUI and cli.
- `ceres keys add` takes secret words a prompt on the command line or stdin instead of command line arguments for security.
...
- We now hand build Apple Silicon native binary wheels for all ceres-blockchain dependencies and host them at [https://pypi.ceres.net/simple](https://pypi.ceres.net/simple). We are likely to hand build a MacOS ARM64 dmg available and certainly will for 1.0. You can install natively on M1 now with the `git clone` developer method today. Just make sure Python 3.9 is installed. `python3 --version` works.
...
- We have moved to a new and much higher scalability download.ceres.net to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.ceres.net/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.ceres.net/latest/Setup-MacOS.dmg).
- Ceres wheels not on Pypi and some dependecies not found there also are now on pypi.ceres.net.
...
- PRs and commits to the ceres-blockchain-gui repository will automatically have their locales updated.

## Fixed

- The Farm page will now no longer get stuck at 50 TXCH farmed.
- `ceres farm` has had multiple bugs and spelling issues addressed. Thanks to @alfonsoperez, @soulmerge and @olivernyc for your contributions.
- `ceres wallet` had various bugs.
...
- We now support a "green flag" chain launch process. A new version of the software will poll download.ceres.net/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Thanks to @jespino there is now `ceres farm summary`. You can now get almost exactly the same farming information on the CLI as the GUI.
- We have added Romanian to the GUI translations. Thank you to @bicilis on [Crowdin](https://crowdin.com/project/ceres-blockchain). We also added a couple of additional target languages. Klingon anyone?
- `ceres wallet` now takes get_address to get a new wallet receive address from the CLI.
- `ceres plots check` will list out all the failed plot filenames at the end of the report. Thanks for the PR go to @eFishCent.
...
- Thanks to @xdustinface of Dash, the BlS Signature library has been updated to 0.9 with clean ups and some speed ups. This changed how the G2 infinity element was handled and we now manage it inside of ceres-blockchain, etc., instead of in blspy.
...
- And thanks yet again to @jespino for a host of PRs to add more detailed typing to various components in ceres-blockchain.
...
- Adding Click changed the behavior of `ceres keys add -m`. The help now makes it clear that the 24 word mnemonic needs to be surrounded by a pair of quotes.
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.ceres.net via https, for example.
- The difficulty adjustment calculation was simplified.
- All of the ceres sub repositories that were attempting to build MacOS Universal wheels were only generating x86_64 wheels internally. We have moved back to only generating x86_64 MacOS wheels on CI.
- However, we have updated and test compiled all Ceres dependencies on Apple Silicon and will be making available a test .dmg for MacOS ARM64 shortly.
- Various weight proof edge cases have been fixed.
- Various typos and style clean ups were made to the Click CLI implementation. `ceres -upnp f` was added to disable uPnP.
- `ceres plots check` shouldn't crash when encountering plots that cause RuntimeError. PR again thanks to @eFishCent.
...
- There are now Swedish, Spanish, and Slovak translations. Huge thanks to @ordtrogen (Swedish), @jespino and @dvd101x (Spanish), and our own @seeden (Slovak). Also thanks were due to @f00b4r (Finnish), @A-Caccese (Italian), and @Bibop182 and @LeonidShamis (Russian). Quite a few more are almost complete and ready for inclusion. You can help translate and review translations at our [crowdin project](https://crowdin.com/project/ceres-blockchain).
- You can obtain a new wallet receive address on the command line with `ceres wallet new_address`. Thanks to @jespino for this and a lot more in the next section below.
...
- The entire CLI has been ported to [Click](https://click.palletsprojects.com/en/7.x/). Huge thanks to @jespino for the big assist and @unparalleled-js for the [recommendation and the initial start](https://github.com/Ceres-Network/ceres-blockchain/issues/464). This will make building out the CLI much easier. There are some subtle changes and some shortcuts are not there anymore. `ceres -h` and `ceres SUBCOMMAND -h` can be your guide.
...
- `ceres plots check` now will continue checking after it finds an error in a plot to the total number of checks you specified.
...
- Interface improvements for `ceres netspace`.
- Now that aiosqlite included our upstream improvements we install version 0.17.0.
- `ceres init` only migrates release candidate directories. The versioned sub directories under `~/ceres` will be going away before mainnet.
...
- `ceres configure -log-level` was broken.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/Ceres-Network/ceres-blockchain/releases/tag/1.0rc1) was well.
...
- Use 'ceres wallet get_transactions' in the command line to see your transactions.
- 'ceres wallet show' now shows your wallet's height.
...
- We have integrated with crowdin to make it easier to translate the GUI. Check out [Ceres Blockchain GUI](https://crowdin.com/project/ceres-blockchain) there.
- We have added Italian, Russian, and Finnish. More to come soon.
- There is now remote UI support. [Documents](https://github.com/Ceres-Network/ceres-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/Ceres-Network/ceres-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- Added `ceres plots check --challenge-start [start]` that begins at a different `[start]` for `-n [challenges]`. Useful when you want to do more detailed checks on plots without restarting from lower challenge values you already have done. Huge thanks to @eFishCent for this and all of the debugging work behind the scenes confirming that plot failures were machine errors and not bugs!
...
- 'ceres wallet send' now takes in TXCH or XCH as units instead of mojos.
...
- You can now send and receive transactions with the command line. Try `ceres wallet -h` to learn more. Also, `ceres wallet` now requires a third argument of `show`, therefor you will use `ceres wallet show` to see your wallet balance.
- We have added the [Crowdin](https://crowdin.com/) translation platform to [ceres blockchain gui](https://crowdin.com/project/ceres-blockchain). We are still getting it fully set up, but helping to translate the GUI is going to be much easier.
...
- `ceres init -c [directory]` will create new TLS certificates signed by your CA located in `[directory]`. Use this feature to configure a new remote harvester. Type `ceres init -h` to get instructions. Huge thanks to a very efficient @eFishCent for this quick and thorough pull request.
...
- The GUI is now in a separate submodule repository from ceres-blockchain, [ceres-blockchain-gui](https://github.com/Ceres-Network/ceres-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `ceres-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- The cli wallet, `ceres wallet`, was incorrectly displaying TXCH balances as if they were Coloured Coins.
...
- We have made the "Help Translate" button in the GUI open in your default web browser and added instructions for adding new translations and more phrases in existing translations at that [URL](https://github.com/Ceres-Network/ceres-blockchain/tree/main/electron-react/src/locales). Try the "Help Translate" option on the language selection pull down to the left of the dark/light mode selection at the top right of the GUI.
...
- We are down to only one sub dependency that does not ship binary wheels for all four platforms. The only platform still impacted is ARM64 (generally Raspberry Pi) but that only means that you still need the minor build tools as outlined on the [wiki](https://github.com/Ceres-Network/ceres-blockchain/wiki/Raspberry-Pi).
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/Ceres-Network/ceres-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- `ceres plots check` enforces a minimum of `-n 5` to decrease false negatives. Thanks to @eFishCent for these ongoing pull requests!
...
- We have removed the '-r' flag from `ceres` as it was being overridden in most cases by the `-r` for restart flag to `ceres start`. Use `ceres --root-path` instead.
- `ceres -h` now recommends `ceres netspace -d 192` which is approximately one hours worth of sub blocks. Use `-d 1000` to get the same estimate of netspace as the RPC and GUI.
- `ceres show -c` now displays in MiB and the GUI has been changed to MiB to match.
- `ceres configure` now accepts the shorter `-upnp` and `-log-level` arguments also.
- `ceres plots check` now defaults to `-n 30` instead of `-n 1` - HT @eFishCent.
- `ceres plots create` now enforces a minimum of k=22. As a reminder, anything less than k=32 is just for testing and be careful extrapolating performance of a k less than 30 to a k=32 or larger.
...
- `ceres show -c` was showing upload MiB in the download column and vice versa. @pyl and @psydafke deserves credit for insisting it was broken and @kd637xx for the PR assist.
- `ceres show` handles sub block 0 better.
...
- `ceres plots create -x` skips adding [final dir] to harvester for farming
...
- Removed loading plot file names when starting `ceres plots create`; decreases plotter time when there are a lot of plots on the machine. Huge thanks to @eFishCent for this PR!
...
- `ceres show -h` now correctly states that you use the first 8 characters of the node id to remove a node on the cli.
- Thank you to @wallentx for adding better help for `ceres configure --enable-upnp`.
...
- `ceres configure` now lets you enable or disable uPnP.
...
- `ceres netspace -s` would not correctly look up the start block height by block hash. Additionally netspace now flips to PiB above 1024 TiB. To compare netspace to `ceres show` of the GUI use `ceres netspace -d 1000` as `ceres netspace` defaults to `-d 192` which is one hour.
...
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [ceres.net](https://ceres.net/). Among the improvements this gives the Ceres blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
...
- You can now specify which private key to use for `ceres plots create`. After obtaining the fingerprint from `ceres keys show`, try `ceres plots create -a FINGERPRINT`. Thanks to @eFishCent for this pull request!
...
- There is a new cli command - `ceres configure`. This allows you to update certain configuration details like log level in config.yaml from the command line. This is particularly useful in containerization and linux automation. Try `ceres configure -h`. Note that if ceres services are running and you issue this command you will have to restart them for changes to take effect but you can use this command in the venv when no services are running or call it directly by path in the venv without activating the venv. Expect the options for this command to expand.
...
- Harvester and farmer will start when the GUI starts instead of waiting for key selection if there are already keys available. This means you will start farming on reboot if you have the Ceres application set to launch on start.
...
- RPCs and daemon now communicate over TLS with certificates that are generated into `~/.ceres/VERSION/config/`
...
- `ceres show -s` has been refactored to support the new consensus.
- `ceres netspace` has been refactored for new consensus.
...
- Tests now place a cache of blocks and plots in the ~/.ceres/ directory to speed up total testing time.
- Changes were made to chiapos to correctly support the new bitfiled backpropogation on FreeBSD and OpenBSD. With the exception of needing to work around python cryptography as outlined on the wiki, FreeBSD and OpenBSD should be able to compile and run ceres-blockchain.
...
- `ceres show -s` now is less brain dead when a node is initially starting to sync.
...
- We have bitfield optimized phase 2 of plotting. There is only about a 1% increase in speed from this change but there is a 12% decrease in writes with a penalty of 3% more reads. More details in [PR 120](https://github.com/Ceres-Network/chiapos/pull/120). Note that some sorts in phase 2 and phase 3 will now appear "out of order" and that is now expected behavior.
- Partial support for Python 3.9. That includes new versions of Ceres dependencies like chiabip158.
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/Ceres-Network/ceres-blockchain/issues/500).
...
- FreeBSD and OpenBSD should be able to build and install ceres-blockchain and its dependencies again.
...
- In the GUI there was [a regression](https://github.com/Ceres-Network/ceres-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/Ceres-Network/ceres-blockchain/issues/485) where log level could no longer be set by service.

## [1.0beta16] aka Beta 1.16 - 2020-10-20

### Added

- The Ceres GUI now supports dark and light mode.
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/Ceres-Network/ceres-blockchain/tree/dev/electron-react/src/locales) of the ceres-blockchain repository.
- `ceres check plots` now takes a `-g` option that allows you to specify a matching path string to only check a single plot file, a wild card list of plot files, or all plots in a single directory instead of the default behavior of checking every directory listed in your config.yaml. A big thank you to @eFishCent for this pull request!
...
- Updated to chiapos 0.12.32. This update significantly speeds up the F1/first table plot generation. It also now can log disk usage while plotting and generate graphs. More details in the [chiapos release notes](https://github.com/Ceres-Network/chiapos/releases/tag/0.12.32).
...
- The rpc interfaces of all ceres services have been refactored, simplified, and had various additional functionality added.
- Block timestamps are now stored in the wallet database. Both database versions were incremented and databases from previous versions will not work with Beta 14. However, upon re-sync all test ceres since Beta 12 should appear in your wallet.
...
- Long_description_content_type is now set to improve ceres-blockchian's Pypi entry. Thanks to @altendky for this pull request.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from Ceres Network at backup.ceres.net. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the Ceres backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.ceres.net, your own installation, or a third party's version of it.
- Added a Code of Conduct in CODE_OF_CONDUCT.md.
- Added a bug report template in `.github/ISSUE_TEMPLATE/bug_report.md`.

### Changed

- This is a new blockchain as we changed how the default puzzle hashes are generated and previous coins would not be easy to spend. Plots made with Beta 8 and newer continue to work, but all previous test ceres are left on the old chain and do not migrate over. Configuration data like plot directories automatically migrate in your `~/.ceres` directory.
...
- `ceres init` now automatically discovers previous releases in each new release.

### Fixed

- `ceres show -w` should now more reliably work. Wallet balances should be more often correct.
...
- Fixed a typo in `ceres show -w` with thanks to @pyl on Keybase.
- In Windows the start menu item is now Ceres Network and the icon in Add/Remove is updated.

## [1.0beta11] aka Beta 1.11 - 2020-08-24

### Added

- The Ceres UI now has a proper About menu entry that gives the various component versions and directs people to submit issues on GitHub. Thank you to @freddiecoleman for this pull request!
- Ability to run only the farmer, wallet, or timelord services, for more advanced configurations (ceres run farmer-only, wallet-only, timelord-only)
...
- `ceres show -w` now correctly outputs all wallet types and balances from the local wallet.
...
- `ceres show -b` now returns plot public key and pool public key for each block.
- Added cbor2 binary wheels for ARM64 to the Ceres simple site. Raspberry Pi should be just a little easier to install.

### Changed

- Wallet addresses and other key related elements are now expressed in Chech32 which is the Ceres implementation of [Bech32](https://github.com/bitcoin/bips/blob/master/bip-0173.mediawiki). All of your old wallet addresses will be replaced with the new Chech32 addresses. The only thing you can't do is send test ceres between 1.8/1.9 and 1.10 software. Anyone who upgrades to 1.10 will keep their transactions and balances of test ceres from the earlier two releases however.
...
- `ceres show -w` now displays a message when balances cannot be displayed instead of throwing an error. Thanks to @freddiecoleman for this fix!
- Fix issue with shutting down full node (full node processes remained open, and caused a spinner when launching Ceres)
...
- See wallet balances in command line: `ceres show -w`
- Retry opening invalid plots every 20 minutes (so you can copy a large plot into a plot directory.)
- We've added `ceres keys sign` and `ceres keys verify` to allow farmers to certify their ownership of keys.
...
- `ceres start wallet-server` changed to `ceres start wallet`, for consistency.
...
- Removed legacy scripts such as ceres-stop-server, ceres-restart-harvester, etc.
...
are read.
- Updated [blspy](https://github.com/Ceres-Network/bls-signatures) to use the
...
- The ceres executable is now available if installing from the Windows or MacOS
Graphical installer. Try `./ceres -h` from
`~\AppData\Local\Ceres-Blockchain\app-0.1.8\resources\app.asar.unpacked\daemon\`
in Windows or
`/Applications/Ceres.app/Contents/Resources/app.asar.unpacked/daemon` on MacOS.
...
"ceres plots create" and "ceres plots check".
- Add plot directories to config.yaml from the cli with "ceres plots add".
...
- The Windows installer is now signed by a Ceres Network certificate. It may take some time to develop enough reputation to not warn multiple times during install.
...
- Test plots for the simulation and testing harness now go into `~/.ceres/test-plots/`
- We have completely refactored all networking code towards making each Ceres service use the same default networking infrastructure and move to websockets as the default networking wire protocol.
- We added additional improvements and more RPCs to the start daemon and various services to continue to make ceres start/stop reliable cross platform.
...
- We now codesign the Apple .dmg installer with the Ceres Network developer ID on both GitHub Actions and Azure Pipelines. We will be notarizing and distributing the Azure Pipelines version as it's built on MacOS Mojave (10.14.6) for stronger cross version support.
...
- Starting ceres now waits for the full node to be active before contacting the introducer.
...
- You can now create or import private keys with a 24 word mnemonic, both in the UI and 'ceres keys' command line.
...
- We added a `ceres keys` command line program, to see, add, and remove private keys.
...
- We added [lgtm](https://lgtm.com/) source analysis on pull request to the ceres-blockchain, chiapos, chiavdf, chiabip158, and bls-library repositories to add some automated security analysis to our ci.
...
- `ceres netspace` has been refactored to use the `/get_network_space` RPC. The command
...
- `ceres-check-plots` uses the plot root and checks the plots in the same manner as harvester.
- `ceres-check-plots` now does not override plots.yaml, which means concurrent plots will properly be added to plots.yaml.
...
- We have added an option to `ceres-create-plots` to specify the second temporary directory. Creating a plot is a three step process. First a working file ending in `.dat.tmp` is created. This file is usually 5 times larger than the final plot file. In the later stages of plotting a second temp file is created ending in `.dat.2.tmp` which will grow to the size of the final plot file. In the final step, the `.dat.2.tmp` is copied to the final `.dat` plot file. You can now optionally set the directory for the `.dat.2.tmp` file with the `-2` flag. An example use case is plotting on a ramdisk and writing both the second temp file and the final file out to an SSD - `ceres-create-plots -n 1 -k 30 -t /mnt/ramdisk -2 /mnt/SSD -d /mnt/SSD`.

### Changed

- `ceres init` properly migrates from previous versions including the k>=32 workaround. Additionally, the farming target key is checked to make sure that it is the valid and correct public key format.
- We have implemented a workaround for the `ceres start` issues some were having upon crash or reboot. We will be rebuilding start and stop to be robust across platforms.
- This release re-includes `ceres-start-harvester`.
...
- The user interface now refers to ceres coins with their correct currency code of XCH.
...
- There was a regression in Ceres Proof of Space ([chiapos](https://github.com/Ceres-Network/chiapos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
...
- Now you can run estimates of the total space currently farming the network. Try `ceres netspace -d 12` to run an estimate over the last 12 blocks which is approximately 1 hour.
- We’ve added TLS authentication for incoming farmer connections. TLS certs and keys are generated during ceres init and only full nodes with your keys will be able to connect to your Farmer. Also, Harvester, Timelord, and Wallet will now not accept incoming connections which reduces the application attack surface.
- The node RPC has a new endpoint get_header_by_height which allows you to retrieve the block header from a block height. Try `ceres show -bh 1000` to see the block header hash of block 1000. You can then look up the block details with `ceres show -b f655e1a9f7f8c89a703e40d9ce82ae33508badaf7b37fa1a56cad27926b5e936` which will look up a block by it's header hash.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/Ceres-Network/ceres-blockchain/wiki) and placed in [INSTALL](https://github.com/Ceres-Network/ceres-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/Ceres-Network/ceres-blockchain/wiki/Quick-Start-Guide)
...
- Most scripts have been removed in favor of ceres action commands. You can run `ceres version` or `ceres start node` for example. Just running `ceres` will show you more options. However `ceres-create-plots` continues to use the hyphenated form. Also it's now `ceres generate keys` as another example.
- Ceres start commands like `ceres start farmer` and `ceres stop node` now keep track of process IDs in a run/ directory in your configuration directory. `ceres stop` is unlikely to work on Windows native for now. If `ceres start -r node` doesn't work you can force the run/ directory to be reset with `ceres start -f node`.
- We suggest you take a look at our [Upgrading documentation](https://github.com/Ceres-Network/ceres-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- The script `ceres start wallet-gui` has been chaned to `ceres start wallet` which launches but the GUI and server on MacOS and Linux. `ceres start wallet-server` remains for WSL 2 and Windows native.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/Ceres-Network/ceres-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `ceres stop all` command sometimes fails, use `ceres-stop-all` instead. In windows, use the task manager to stop the servers.
...
- We added full block lookup to `ceres show`.

### Changed

- `ceres-restart-harvester` has been renamed from `ceres-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- If you previously used the plot_root variable in config, your plot directory names might not migrate correctly. Please double check the filenames in `~/.ceres/beta-1.0b3/config/plots.yaml` after migrating
...
- There is now full transaction support on the Ceres blockchain. In this initial Beta 1.0 release, all transaction types are supported though the wallets and UIs currently only directly support basic transactions like coinbase rewards and sending coins while paying fees. UI support for our [smart transactions](https://github.com/Ceres-Network/wallets/blob/main/README.md) will be available in the UIs shortly.
...
- Ceres fullnode, farmer and harvester now default to logging to ceres.log in the ceres-blockchain directory. This is configured in config.yaml and due to config.yaml changes it is recommended to edit the new template config instead of using older config.yaml’s from previous versions.
...
- VDF improvements - VDF verification and generation speed has increased and dependence on flint2 has been removed. We wish to thank Dr. William Hart (@wbhart) for dual licensing parts of his contributions in FLINT and Antic for inclusion in the Ceres blockchain.
...
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to chia_full node, chia_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new ceres services were started.
...
- This is the first release of the Ceres testnet! Blockchain consensus, proof of time, and proof of space are included.
- More details on the release at [https://www.ceres.net/developer/](https://www.ceres.net/developer/)

[unreleased]: https://github.com/Ceres-Network/ceres-blockchain/compare/1.0beta5...dev
[1.0beta5]: https://github.com/Ceres-Network/ceres-blockchain/compare/1.0beta4...1.0beta5
[1.0beta4]: https://github.com/Ceres-Network/ceres-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/Ceres-Network/ceres-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/Ceres-Network/ceres-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/Ceres-Network/ceres-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/Ceres-Network/ceres-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/Ceres-Network/ceres-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/Ceres-Network/ceres-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/Ceres-Network/ceres-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/Ceres-Network/ceres-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/Ceres-Network/ceres-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/Ceres-Network/ceres-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/Ceres-Network/ceres-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/Ceres-Network/ceres-blockchain/releases/tag/Alpha-1.0
