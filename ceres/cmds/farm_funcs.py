...
from ceres.rpc.full_node_rpc_client import FullNodeRpcClient
...
from ceres.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total ceres farmed: {amounts['farmed_amount'] / units['ceres']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['ceres']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['ceres']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'ceres start wallet' and 'ceres wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'ceres wallet show'")
    else:
        print("Note: log into your key using 'ceres wallet show' to see rewards for each key")
