...
def show_plots(root_path: Path):
    from ceres.plotting.plot_tools import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'ceres plots add -d [dir]' and remove with"
        + " 'ceres plots remove -d [dir]'"
        + " Scan and check plots with 'ceres plots check'"
    )
    print()
...
def plots_cmd(ctx: click.Context):
    ...
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with 'ceres init'")
...
def create_cmd(
    ...
    from ceres.plotting.create_plots import create_plots
...
def check_cmd(
    ...
    from ceres.plotting.check_plots import check_plots
...
def add_cmd(ctx: click.Context, final_dir: str):
    from ceres.plotting.plot_tools import add_plot_directory
...
def remove_cmd(ctx: click.Context, final_dir: str):
    from ceres.plotting.plot_tools import remove_plot_directory
