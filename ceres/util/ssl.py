...
import stat
import sys
from ceres.util.config import load_config, traverse_dict
from ceres.util.permissions import octal_mode_string, verify_file_permissions
...
def check_ssl(root_path: Path) -> None:
    ...
    if len(invalid_files):
        ...
        print("Run `ceres init --fix-ssl-permissions` to fix issues.")
