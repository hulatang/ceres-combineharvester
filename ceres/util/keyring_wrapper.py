...
from ceres.util.default_root import DEFAULT_KEYS_ROOT_PATH
from ceres.util.file_keyring import FileKeyring
from ceres.util.misc import prompt_yes_no
...
DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE = "$ ceres passphrase set # all the cool kids are doing it!"
...
class KeyringWrapper:
    ...
    def set_master_passphrase(
        ...
        from ceres.util.keychain import KeyringCurrentPassphaseIsInvalid, KeyringRequiresMigration
    ...
    def confirm_migration(self) -> bool:
        ...
        if master_passphrase == DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE:
            ...
            print(
                "Would you like to set a master passphrase now? Use 'ceres passphrase set' to change the passphrase.\n"
            )

            response = prompt_yes_no("Set keyring master passphrase? (y/n) ")
            if response:
                from ceres.cmds.passphrase_funcs import prompt_for_new_passphrase
                ...
                print(
                    "Will skip setting a master passphrase. Use 'ceres passphrase set' to set the master passphrase.\n"
                )
        else:
            import colorama
    ...
    def migrate_legacy_keys(self) -> MigrationResults:
        from ceres.util.keychain import Keychain, MAX_KEYS
    ...
    def migrate_legacy_keyring_interactive(self):
        ...
        """
        from ceres.cmds.passphrase_funcs import async_update_daemon_migration_completed_if_running
