...
from ceres.cmds.init_funcs import check_keys, chia_init
from ceres.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from ceres.daemon.keychain_server import KeychainServer, keychain_commands
from ceres.daemon.windows_signal import kill
from ceres.server.server import ssl_context_for_root, ssl_context_for_server
...
from ceres.util.json_util import dict_to_json_str
...
from ceres.util.service_groups import validate_service
from ceres.util.setproctitle import setproctitle
from ceres.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Ceres.")
    quit()
...
service_plotter = "ceres plots create"
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("ceres daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered CERES_ROOT
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with CERES_ROOT: {os.environ['CERES_ROOT']}")
